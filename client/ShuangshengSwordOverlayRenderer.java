...
public class ShuangshengSwordOverlayRenderer {
    ...
    public static void onScreenRender(ScreenEvent.Render.Post event) {
        ...
        for (Slot slot : containerScreen.getMenu().slots) {
            if (!slot.hasItem() || !slot.getItem().is((Item)ModItems.SHUANGSHENG_SWORD.get())) continue;
            ...
            ShuangshengSwordOverlayRenderer.renderHalo(event.getGuiGraphics(), cx, cy, hovered);
        }
    }
    ...
    public static void onRenderGuiOverlay(RenderGuiOverlayEvent.Pre event) {
        ...
        if (mainHand.is((Item)ModItems.SHUANGSHENG_SWORD.get())) {
        ...
        if (!found && mc.player.getOffhandItem().is((Item)ModItems.SHUANGSHENG_SWORD.get())) {
            int sw = mc.getWindow().getGuiScaledWidth();
            int sh = mc.getWindow().getGuiScaledHeight();
        ...
        ShuangshengSwordOverlayRenderer.renderHalo(event.getGuiGraphics(), cx, cy, false);
    }
    ...
    private static void renderHalo(GuiGraphics guiGraphics, float cx, float cy, boolean hovered) {
        ...
        ShuangshengSwordOverlayRenderer.renderRingWave(vc, matrix, time);
        for (int i = 0; i < 9; ++i) {
            float angle = time * 1.0f + (float)((double)i * 2.0 * Math.PI / 9.0);
            poseStack.pushPose();
            poseStack.mulPose(Axis.ZP.rotation(angle));
            boolean isBlue = i % 2 == 0;
            ColorInfo color = ShuangshengSwordOverlayRenderer.generateTimeBasedColor(time, i, 9, isBlue);
            ShuangshengSwordOverlayRenderer.renderGlowRay(vc, poseStack.last().pose(), 1.15f, 0.02f, 0.525f, color);
            poseStack.popPose();
        }
    ...
    private static void renderRingWave(VertexConsumer vc, Matrix4f matrix, float time) {
        int alpha = 70;
        for (int i = 0; i < 42; ++i) {
            int nextI = (i + 1) % 42;
            float angle1 = (float)(Math.PI * 2 * (double)i / 42.0);
            float angle2 = (float)(Math.PI * 2 * (double)nextI / 42.0);
            float offset1 = ShuangshengSwordOverlayRenderer.calculateWaveOffset(time * 4.0f, i);
            float offset2 = ShuangshengSwordOverlayRenderer.calculateWaveOffset(time * 4.0f, nextI);
    ...
    private static ColorInfo generateTimeBasedColor(float time, int index, int count, boolean isBlue) {
        ...
        float baseHue = isBlue ? 0.58f : 0.08f;
        float hueRange = 0.08f;
        float h1 = (hue1 / 360.0f * hueRange + baseHue) % 1.0f;
        float h2 = (hue2 / 360.0f * hueRange + baseHue) % 1.0f;
        int color1 = Color.HSBtoRGB(h1, 1.0f, 1.0f);
        int color2 = Color.HSBtoRGB(h2, 1.0f, 1.0f);
