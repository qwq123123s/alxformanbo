...
public class BeyondCoreOverlayHandler {
    private static boolean tooltipActive = false;
    private static long lastTooltipTime = 0L;
    private static final long TOOLTIP_TIMEOUT_MS = 20L;
    ...
    public static void onTooltipPre(RenderTooltipEvent.Pre event) {
        if (event.getItemStack().is((Item)ModItems.MANBO_BEYOND_CORE.get())) {
            tooltipActive = true;
    ...
    public static void onScreenRenderPost(ScreenEvent.Render.Post event) {
        ...
        int r = 255;
        int g = (int)(180.0 + breath * 75.0);
        int bVal = (int)(breath * 40.0);
        ...
        BeyondCoreOverlayHandler.drawHollowStarTriangles(buffer, matrix, lastMouseX, lastMouseY, outerRadius, innerRadius, rotationAngle, r, g, bVal, alphaMain, 0.0f, 3.0f);
        tesselator.end();
        ...
        buffer.begin(VertexFormat.Mode.TRIANGLES, DefaultVertexFormat.POSITION_COLOR);
        BeyondCoreOverlayHandler.drawHollowStarTriangles(buffer, matrix, lastMouseX, lastMouseY, glowOuter, glowInner, glowRot, r, g, bVal, alphaGlow, 0.0f, 3.0f);
        tesselator.end();
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (tooltipActive && System.currentTimeMillis() - lastTooltipTime > 20L) {
            tooltipActive = false;
        }
    }
}
